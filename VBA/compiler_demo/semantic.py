...
class BaseType(Enum):
    ...
    #VOID = 'void' in VBA doesn't have VOID type
    INTEGER = 'Integer'
    DOUBLE = 'Double'
    BOOL = 'Boolean'
    STR = 'String'
...
INTEGER, DOUBLE, BOOL, STR, CHAR = BaseType.INTEGER, BaseType.DOUBLE, BaseType.BOOL, BaseType.STR, BaseType.CHAR
    ...
    def add_ident(self, ident: IdentDesc) -> IdentDesc:
        ...
        if (temp):
            ...
            if ident.scope == ScopeType.PARAM:
                ...
                if temp.scope not in (ScopeType.GLOBAL):
                    error = True
            else:
                error = True
            if error:
