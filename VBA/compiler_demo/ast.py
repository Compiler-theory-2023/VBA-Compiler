...
class CaseNode(StmtNode):
    """Класс для представления в AST-дереве case оператора
    """

    def __init__(self, cond: ExprNode, body: StmtNode,
                 row: Optional[int] = None, col: Optional[int] = None, **props) -> None:
        super().__init__(row=row, col=col, **props)
        self.cond = cond if isinstance(cond, AstNode) else None
        self.body = body

    def __str__(self) -> str:
        return 'case' if self.cond else 'default'

    @property
    def childs(self) -> tuple[ExprNode, StmtNode]:
        return ((self.cond, ) if self.cond else tuple()) + (self.body, )
...
class SwitchNode(StmtNode):
    ...
    def __init__(self, cond: ExprNode, *cases: StmtNode,
                 row: Optional[int] = None, col: Optional[int] = None, **props) -> None:
        super().__init__(row=row, col=col, **props)
        self.cond = cond
        self.cases = cases
    ...
    def semantic_check(self, scope: IdentScope) -> None:
        self.cond.semantic_check(scope)
        self.cond = type_convert(self.cond, TypeDesc.BOOL, None, 'условие')
        self.cases.semantic_check(IdentScope(scope))

        self.node_type = TypeDesc.VOID


class WhileNode(StmtNode):
    """Класс для представления в AST-дереве while оператора
    """
...
class DoNode(StmtNode):
    """Класс для представления в AST-дереве do оператора
       """
